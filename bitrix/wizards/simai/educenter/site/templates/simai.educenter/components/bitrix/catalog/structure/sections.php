...
<div class="row  d-flex align-items-end">
  ...
  $APPLICATION->IncludeComponent(
  	...
  	Array(
		...
		"IBLOCK_ID_BRANCHES" => $arParams["IBLOCK_ID_BRANCHES"],
		"IBLOCK_ID_DEPARTMENTS" => $arParams["IBLOCK_ID_DEPARTMENTS"],
  		"DISPLAY_COMPARE" => "N",
  		"SET_TITLE" => "N",
  		"SET_STATUS_404" => "N",
...
<?if($arParams["AJAX_PAGE"]=="Y")$GLOBALS['APPLICATION']->RestartBuffer();?>
 ...
 <?
if($_REQUEST["structure"]||$_REQUEST["program"])
{?>
    <script type="text/javascript">
        $(function(){
            $("select[name='structure'] option[value='<?=$_REQUEST["structure"]?>']").attr("selected","");
			$("select[name='program'] option[value='<?=$_REQUEST["program"]?>']").attr("selected","");
        })
    </script>
    <?
	...
	if($_REQUEST["structure"])
      $GLOBALS[$arParams["FILTER_NAME"]]["SECTION_CODE"]= $_REQUEST["structure"];
	  
	if($_REQUEST["program"])
	   $GLOBALS[$arParams["FILTER_NAME"]]["PROPERTY_PROGRAM"]= $_REQUEST["program"];
...
$APPLICATION->IncludeComponent(
    ...
    Array(
        ...
        "IBLOCK_ID" => $arParams["IBLOCK_ID"],
		"IBLOCK_DISCIPLINE" => $arParams["IBLOCK_DISCIPLINE"],
