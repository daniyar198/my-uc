...
if($arParams["TYPE"] || !in_array($arParams["DATE_FIELD"], array("DATE_ACTIVE_FROM", "DATE_ACTIVE_TO", "TIMESTAMP_X", "DATE_CREATE", "PROPERTY_DATE")))
	$arParams["DATE_FIELD"] = "PROPERTY_DATE_VALUE";
...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
	...
	$arSelectedFields = Array("ACTIVE", "PROPERTY_DATE", "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL", "NAME", "LANG_DIR", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE","PROPERTY_COURSE");

	$dbItems = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelectedFields);
	...
	while($arItem = $dbItems->GetNext())
	{
		...
		$arCourse[$arItem["PROPERTY_COURSE_VALUE"]] = $arItem["PROPERTY_COURSE_VALUE"];
	}

	
	$arSelect = Array("ID", "NAME", "CODE","IBLOCK_ID","IBLOCK_SECTION_ID","DETAIL_PAGE_URL");
	$arFilter = Array("IBLOCK_ID" => $arParams["IBLOCK_ID_COURSE"],"ID" => $arCourse);
	$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
	while($ob = $res->GetNextElement())
	{
	 $arFields = $ob->GetFields();
	 $arCourseDetail[$arFields["ID"]] =array("NAME" => $arFields["NAME"], "DETAIL_PAGE_URL"  => $arFields["DETAIL_PAGE_URL"]);
	}

	
	
	
	foreach($arDays as $kday => $dayVal){
	 foreach($dayVal as $kCourse => $courseVal){
		
		$dayVal[$kCourse]["DETAIL_PAGE_URL"] = $arCourseDetail[$courseVal["PROPERTY_COURSE_VALUE"]]["DETAIL_PAGE_URL"];
		$dayVal[$kCourse]["NAME"] = $arCourseDetail[$courseVal["PROPERTY_COURSE_VALUE"]]["NAME"];
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
					...
					$arDay["events"][] = array(
						...
						"title"=>$title,
						"preview"=>$sTitle,
						"PROPERTY_DATE" =>$dayNews["PROPERTY_DATE"],
					);
