...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<div class="form-group" <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"):?> style="display: none;"<?endif?>>
					<label><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span style="color:red">*</span><?endif?></label>
						...
						<textarea class="form-control" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
								<input class="form-control" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime" && !$value):?>value="<?=date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")), time());?>"<?else:?>value="<?=$value?>"<?endif?> size="25"  /><?
						...
						<input class="form-control" type="file" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>"  name="PROPERTY_FILE_<?=$propertyID?>_<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i?>" /><br />
									<?
			...
			<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
				<div class="form-group">
						...
						<label><?=GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT")?><span style="color:red">*</span></label>
 ...
 <div class="mt-10 mb-10">
			...
			<tr>
				<td colspan="2">
					<input type="submit" name="iblock_submit" class="btn btn-base" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
