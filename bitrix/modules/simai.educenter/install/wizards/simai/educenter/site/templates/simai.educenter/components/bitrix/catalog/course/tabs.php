...
<ul class="tabs clearfix">
  ...
  <li><a data-toggle="tab" href="#shedule"><?=getMessage("SHEDULE")?></a></li>
</ul>
					...
					<tbody>
					   ...
					   <?foreach($arBXCSiteSettings["FORM"] as $keyForm=> $arForm):?>
						<tr>
						  ...
						  <td><?=$arForm["SUB_VALUES"]["COST_LESSON_FORM"]["~VALUE"]?></td>
						  ...
						  <td>
						   ...
						   <td><a href="#" onclick="SimaiAdd2Basket(<?=$arBXCSiteSettings["ID"]?>,<?=$keyForm?>)"
							class="btn btn-success btn-outline waves-effect waves-light"><?=getMessage("BUY")?></a></td>
						</tr>
					  <?endforeach;?>
...
<div id="shedule" class="tab-pane fade">
 <table class="table table-hover stacktable large-only">
    <thead>
        <tr>
		    <th><?=GetMessage('EDUCATION_FORMS')?></th>
