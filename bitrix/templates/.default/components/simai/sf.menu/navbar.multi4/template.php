...
<?//=========== TOP.MENU & LEFT.MENU ==============?>
            ...
            <?$previousLevel = 0;
                ...
                // Показываем только разделы первого уровня на странице "Все курсы" (левое меню)
                if (
                    $arParams["SF_MODE_NAVBAR"] == "LEFT" &&
                    strpos($APPLICATION->GetCurPage(), '/courses/') !== false &&
                    !$arParams['SECTION_ID'] &&
...
echo 'URL: ' . $APPLICATION->GetCurPage() . '<br>';
echo 'SF_MODE_NAVBAR: ' . (isset($arParams["SF_MODE_NAVBAR"]) ? $arParams["SF_MODE_NAVBAR"] : 'нет') . '<br>';
echo 'SECTION_ID: ' . (isset($arParams['SECTION_ID']) ? $arParams['SECTION_ID'] : 'нет') . '<br>';
echo '</div>';
?>
