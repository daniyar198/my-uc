...
<?foreach($arResult["FIELDS"] as $field_code=>$field_name):
	...
	<?else:?>
	<?if($field_code != "DOCUMENT"):?>
        <div class="form-group">
            
            <?if (array_key_exists($field_code,$arResult["FIELDS_TXT"])):?>
                <textarea style="display: none" class="form-control" name="PROP[<?=$field_code?>]"><?=htmlspecialchars(strip_tags($_POST["PROP"][$field_code]),ENT_QUOTES)?></textarea>
            <?else:?>
                <label>
					<?=$field_name["NAME"]?>
                ...
                <input class="form-control" name="PROP[<?=$field_code?>]" type="text" value="<?=htmlspecialchars(strip_tags($_POST["PROP"][$field_code]),ENT_QUOTES)?>">
            <?endif?>
        </div>
	<?endif;?>
	<?endif;
